...
**Watch** opens an unobstructed, full-viewport view and requests browser fullscreen when available. It keeps the current map and filters, starts motion at one recorded minute per second, quiets geography, and adds soft aircraft halos in both renderers. Labels, panels, movement rings, selected-route overlays and the timeline disappear. The pointer and exit hint fade after inactivity; move, tap or Tab to reveal the exit, press Escape to leave, or Space to pause. Exit restores the previous playback mode and pace without rewinding the clock. Browser fullscreen denial falls back to the same distraction-free layout within the page.
