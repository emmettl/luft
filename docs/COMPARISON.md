...
Choose **View → Aircraft renderer** to switch without losing the clock, map view, carrier or airport. Canvas remains the default. Three.js loads on demand. WebGL failure or context loss returns to Canvas with a visible explanation. **Device details** shows the version; refresh an older tab before comparing.
