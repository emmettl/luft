...
export function FlightSearch({airports,routes,selection,onChange,onAirportBoard}:Props){
 ...
 return <div ref={root} className="flight-search">
  <div className="unified-input"><svg aria-hidden="true" width="17" height="17" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.4"><circle cx="8.5" cy="8.5" r="5.5"/><path d="m13 13 4 4"/></svg><input ref={input} aria-label="Search flights" placeholder="Airports, airlines or routes" autoComplete="off" role="combobox" aria-expanded={open} aria-controls="flight-results" aria-activedescendant={open&&results[active]?`flight-result-${active}`:undefined} value={query} onFocus={()=>setOpen(true)} onChange={e=>{setQuery(e.target.value);setActive(0);setOpen(true)}} onKeyDown={event=>{
