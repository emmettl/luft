...
export function FlightSearch({onClear,onEndpoints,endpointsOpen,endpointActive,countries,airports,routes,selection,onChange,onAirportBoard,onCountryFocus}:Props){
 ...
 const results=useMemo(()=>{
  ...
  return [
   ...
   ...(['countries','continents'] as const).flatMap(type=>q?[...new Set(Object.values(airportGeography).map(a=>type==='countries'?a.country:a.continent).filter((code):code is string=>!!code))].filter(id=>type!=='countries'||!countries.some(c=>c.code===id)).map(id=>({type,id,label:type==='countries'?countryLabel(id):continentLabel(id),detail:'Observed departure or arrival airport'})).filter(r=>!selection[type].includes(r.id)&&`${r.label} ${r.id}`.toLowerCase().includes(q)).slice(0,4):[]),
