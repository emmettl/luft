...
export type AirportLabel={airport:Airport;text:string;selected:boolean;box:MapLabelBox}
...
export function layoutAirportLabels(airports:readonly Airport[],ranks:ReadonlyMap<string,number>,selected:ReadonlySet<string>,retained:ReadonlySet<string>,heightDegrees:number,width:number,height:number,project:(lon:number,lat:number)=>number[],obstacles:readonly MapLabelBox[]):AirportLabel[]{
 ...
 const candidates=airports.flatMap(airport=>{
  ...
  if(!chosen&&(!airport.hasObservedMovements||rank>=rankLimit))return []
  ...
  return [{airport,text,selected:chosen,name:airport.icao,rank,priority:chosen?0:3,retained:retained.has(airport.icao),distance:Math.hypot(x-width/2,y-height/2),box:{left:x-8,right:x-8+labelWidth,top:y-22,bottom:y+22}}]
 })
