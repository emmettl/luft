...
it('unions countries, intersects with airport and airline filters, and excludes overflights with unknown endpoints',()=>{
 ...
 const selection={...EMPTY_SELECTION,countries:['CH','FR']}
 expect(flights.map(t=>matchesSelection(t,selection))).toEqual([true,true,false,false])
 expect(flights.map(t=>matchesSelection(t,{...selection,airlines:['swiss'],airports:['EGLL']}))).toEqual([true,false,false,false])
 expect(matchesSelection(flights[0],{...selection,airports:['LFSB']})).toBe(false)
})
