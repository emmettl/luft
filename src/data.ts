...
import holdingUrl from './generated/holding.json?url'
import type {HoldingIndex} from './holding'
...
export async function loadRelease() {
 ...
 const [index,land,snapshots,countryIndex,holding]=await Promise.all([verifiedJson<Index>(manifest.index),fetch(landUrl).then(async r=>{if(!r.ok)throw new Error('Land context unavailable');return await r.json() as Land}),fetch(snapshotsUrl).then(async r=>{if(!r.ok)throw new Error('Filter activity unavailable');return await r.json() as SnapshotIndex}),fetch(countriesUrl).then(async r=>{if(!r.ok)throw new Error('Country context unavailable');return await r.json() as CountryIndex}),fetch(holdingUrl).then(async r=>{if(!r.ok)throw new Error('Holding context unavailable');return await r.json() as HoldingIndex})])
 if(holding.date!==manifest.date||holding.sourceManifestSha256!==dataLock.manifestSha256)throw new Error('Holding context belongs to another release')
 ...
 return {manifest,index,land,snapshots,countries:countryIndex.countries,enrichment,holding}
}
