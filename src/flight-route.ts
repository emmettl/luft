...
export type RoutePoint=[time:number,longitude:number,latitude:number]
...
export function appendRouteSamples(routes:Map<string,RouteSegments>,id:string,samples:readonly AirSample[],start:number,end:number){
 ...
 for(const sample of samples){
  ...
  const point:RoutePoint=[sample[0],sample[1],sample[2]],segment=segments.at(-1),previous=segment?.at(-1)
