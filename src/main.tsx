...
function Study({release}:{release:Release}) {
 ...
 async function copyMeasurements(){
  const m=metrics.current,report={release:version,renderer,recordedDay:manifest.date,browser:navigator.userAgent,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},time:engine.current.time,speed:engine.current.speed,mode,airline,airport:airport?.icao,view:map.current?.view,drawSamples:map.current?.durations.length,drawCpuMaxMs:Math.max(0,...(map.current?.durations??[])),drawCpuP95Ms:percentile(map.current?.durations??[]),stagesP95Ms:Object.fromEntries((['setup','sampling','geometry','submission'] as const).map(stage=>[stage,percentile(map.current?.stages.map(s=>s[stage])??[])])),paintIntervalP95Ms:percentile(m.intervals),paintRate:m.intervals.length?1000*m.intervals.length/m.intervals.reduce((a,b)=>a+b,0):0,bufferSeconds:m.bufferSeconds,gpu:map.current?.gpuStats(),note:'30 Hz paint cap. CPU timing excludes asynchronous GPU completion. Compare identical views and warmed data.'}
