...
function Study({release}:{release:Release}) {
 const {manifest,index,land,snapshots}=release,canvas=useRef<HTMLCanvasElement>(null),map=useRef<AirMap|null>(null)
 const store=useMemo(()=>new ChunkStore(manifest.chunks),[manifest])
 const engine=useRef({time:7*3600,playing:false,speed:300,chunk:undefined as Chunk|undefined,chunkIndex:-1,targetTime:7*3600,generation:0,waiting:false,airports:[] as Airport[],mode:'motion',flight:'',selectedIds:undefined as Set<string>|undefined,cells:index.cells})
 const [ui,setUi]=useState({time:7*3600,playing:false,waiting:true,total:0,inbound:0,outbound:0,visible:0,cache:0,ms:0,responseBytes:0})
 ...
 const changeSelection=(value:Selection)=>{
  setSelection(value);setFlight('');if(map.current)map.current.selectedFlight=undefined
 ...
 useEffect(()=>{
  ...
  const frame=(now:number)=>{
   ...
   if(now-lastDraw>=32 && e.chunk){
    ...
    const counts=map.current!.draw(tracks,e.time,e.airports,e.mode,cells[Math.floor(e.time/3600)]);lastDraw=now
  ...
  return()=>{stopped=true;rendererGeneration.current++;cancelAnimationFrame(raf);e.generation++;store.dispose();map.current?.dispose();document.removeEventListener('visibilitychange',visibility);document.removeEventListener('keydown',keydown)}
 },[store,land,index])
 ...
 return <main className="study">
  ...
  <footer>
   <div className="transport"><button className="play" aria-keyshortcuts="Space" title="Play / pause (Space)" disabled={mode==='density'||!engine.current.chunk} onClick={togglePlayback}>{ui.playing?'Pause':'Play'}</button><label className="speed-label">Pace <select aria-label="Playback speed" value={speed} onChange={e=>{const s=+e.target.value;setSpeed(s);engine.current.speed=s}}><option value="60">1 min / s</option><option value="300">5 min / s</option><option value="900">15 min / s</option></select></label><button className="settings-toggle" aria-label="View settings" aria-expanded={settings} onClick={()=>setSettings(!settings)}>View <svg aria-hidden="true" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor"><path d="M2 4h12M2 12h12"/><circle cx="6" cy="4" r="2" fill="#101f2b"/><circle cx="10" cy="12" r="2" fill="#101f2b"/></svg></button><span className={`stream-status ${status==='Ready'?'is-ready':''}`} role="status">{status==='Ready'?(ui.playing?'Playing': 'Paused'):status}</span>{status.includes('Retry')&&<button onClick={()=>void seek(engine.current.targetTime)}>Retry</button>}</div>
   <StudyTimeline bins={timelineBins} windowStart={0} windowEnd={86400} time={ui.time} onSeek={time=>{engine.current.playing=false;void seek(time)}} onScrubStart={()=>{engine.current.playing=false;updateUi()}} label={selectionLabel||'Aircraft over Europe'} ariaLabel="Time of day UTC" variant={chartStyle} formatValue={formatActivity} description="5-minute snapshots · UTC"/>
   <div className="colophon"><p><a href="https://www.adsb.lol/">ADSB.lol</a> · <a href="https://opendatacommons.org/licenses/odbl/1-0/">ODbL</a></p><button className="text-button" aria-expanded={diagnostics} onClick={()=>setDiagnostics(!diagnostics)}>Device details</button></div>
