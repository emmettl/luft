...
function Study({release}:{release:Release}) {
 ...
 const [endpointFilter,setEndpointFilter]=useState<EndpointFilter>(EMPTY_ENDPOINT_FILTER),[endpointsOpen,setEndpointsOpen]=useState(false)
 const resolveEndpoint=useMemo(()=>createEndpointResolver(enrichment),[enrichment])
 const changeEndpointFilter=(value:EndpointFilter)=>{setEndpointFilter(value);setFlight('');if(map.current){map.current.selectedFlight=undefined;map.current.resetMotionEffects()}}
 ...
 const baseAircraft=useMemo(()=>index.aircraft.filter(track=>matchesSelection(track,selection,countryIds)),[index,selection,countryIds])
 ...
 const selectionLabel=[endpointFilterTitle(endpointFilter,enrichment.airports.find(a=>a.icao===endpointFilter.airport)?.iata||endpointFilter.airport),selectedCountries.map(c=>c.name).join(' + '),airlineLabel(selection.airlines),selectedAirports.map(a=>a.iata||a.icao).join(' + '),selection.routes.map(id=>routes.find(r=>r.id===id)!.label).join(' + ')].filter(Boolean).join(' · ')
 ...
 useEffect(()=>{
  const e=engine.current;map.current=new AirMap(canvas.current!,land,index.airports,{west:manifest.bounds[0],south:manifest.bounds[1],east:manifest.bounds[2],north:manifest.bounds[3]},accentCanvas.current!,manifest.date,{ranks:labelRanks,onChange:setAirportLabels});map.current.setCountries(countries);let stopped=false,raf=0,last=performance.now(),lastDraw=0,lastUi=0
  ...
  const frame=(now:number)=>{
   ...
   if(now-lastDraw>=32 && e.chunk){
    ...
    const counts=map.current!.draw(tracks,e.time,e.airports,e.mode,cells[Math.floor(e.time/3600)],e.accentClock,e.dimOthers?e.selectedIds:undefined);lastDraw=now
 ...
 useEffect(()=>{
  const elements=[...document.querySelectorAll('.study>header .identity,.study>header .date,.workspace aside,.map-caption,.view-controls,.study>footer,.airport-panel,.view-settings,.diagnostics,.flight-caption,.endpoint-panel')]
 ...
 },[selection,boardOpen,settings,diagnostics,flight,endpointsOpen])
