...
import {matchesSelection,selectionActivity,observedRoutes,airlineLabel,type Selection} from './filters'
import {selectionOptions,selectionFromHash,selectionHash} from './selection-url'
...
function Study({release}:{release:Release}) {
 ...
 const filterOptions=useMemo(()=>selectionOptions(index),[index])
 ...
 const selectionLabel=[airlineLabel(selection.airlines),selection.airports.map(code=>index.airports.find(a=>a.icao===code)?.iata||code).join(' + '),selection.routes.map(id=>routes.find(r=>r.id===id)!.label).join(' + '),selection.countries.map(countryLabel).join(' + '),selection.continents.map(continentLabel).join(' + ')].filter(Boolean).join(' · ')
 ...
 const changeSelection=(value:Selection)=>{
  const url=new URL(location.href);url.hash=selectionHash(value)
  if(url.href!==location.href)history.pushState(null,'',url)
  applySelection(value)
 }
 useEffect(()=>{
  // Also fires on Back/Forward between filter hashes and on manual hash edits.
  const restore=()=>applySelection(selectionFromHash(location.hash,filterOptions))
 ...
 return <main className="study">
  ...
  <div className="workspace">
   ...
   <aside aria-label="Flight selection">
    <div className="selection-tools"><FlightSearch airports={index.airports} routes={routes} selection={selection} onChange={changeSelection} onAirportBoard={choose}/>
    {selectionLabel&&<p className="selection-summary">{summary.aircraft.toLocaleString()} aircraft observed · matching selection{dimOthers&&mode==='motion'?' · other flights dimmed':''}</p>}
    ...
    <Insights index={index} selection={selection} onChange={changeSelection}/>
   </aside>
