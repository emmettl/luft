...
import {AIRLINES,matchesAirline,type AirlineSelection} from './airlines'
...
import './style.css'
const airlineLogos={easyjet:easyjetLogo,swiss:swissLogo,'british-airways':britishAirwaysLogo}
...
function Study({release}:{release:Release}) {
 const {manifest,index,land,airlineSummaries}=release,canvas=useRef<HTMLCanvasElement>(null),map=useRef<AirMap|null>(null)
 const store=useMemo(()=>new ChunkStore(manifest.chunks),[manifest])
 const engine=useRef({time:7*3600,playing:false,speed:300,chunk:undefined as Chunk|undefined,chunkIndex:-1,targetTime:7*3600,generation:0,waiting:false,airport:undefined as Airport|undefined,mode:'motion',flight:'',airline:'all' as AirlineSelection})
 ...
 const [airline,setAirline]=useState<AirlineSelection>('all')
 const airlineInfo=AIRLINES.find(a=>a.id===airline)
 const airlineSummary=airline==='all'?undefined:airlineSummaries.airlines[airline]
 ...
 },[store,land,index,airlineSummaries])
 const chooseAirline=(value:AirlineSelection)=>{setAirline(value);engine.current.airline=value;setFlight('');if(map.current)map.current.selectedFlight=undefined}
 ...
 const maximum=Math.max(1,...activity.map(b=>b.count))
