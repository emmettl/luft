...
function Study({release}:{release:Release}) {
 ...
 const watch=useWatchMode(studySurface,()=>{
  const e=engine.current;watchPlayback.current={mode:e.mode,playing:e.playing,speed:e.speed}
 ...
 const changeEndpointFilter=(value:EndpointFilter)=>{writeFilterUrl(selection,value);setEndpointFilter(value);setFlight('');if(map.current){map.current.selectedFlight=undefined;map.current.resetMotionEffects()}}
 ...
 return <main ref={studySurface} tabIndex={-1} className="study" data-watch={watch.active||undefined} data-watch-awake={watch.awake||undefined} onPointerMove={watch.active?watch.reveal:undefined} onPointerDown={watch.active?watch.reveal:undefined}>
  ...
  {watch.active&&<div className="watch-exit" onFocus={watch.reveal}>
   <button onClick={watch.exit} aria-label="Exit watch mode">Exit watch mode <span aria-hidden="true">↙</span></button>
