...
function Study({release}:{release:Release}) {
 ...
 const engine=useRef({time:7*3600,playing:true,scrubbing:false,speed:300,chunk:undefined as Chunk|undefined,chunkIndex:-1,targetTime:7*3600,generation:0,waiting:false,airports:[] as Airport[],mode:'motion',flight:'',selectedIds:undefined as Set<string>|undefined,cells:index.cells})
 ...
 useEffect(()=>{
  ...
  const frame=(now:number)=>{
   ...
   if(e.playing&&!e.waiting&&!e.scrubbing&&e.mode==='motion')void seek(e.time+dt*e.speed,true)
 ...
 return <main className="study">
  ...
  <footer>
   ...
   <StudyTimeline bins={timelineBins} windowStart={0} windowEnd={86400} time={ui.time} onSeek={time=>void seek(time)} onScrubStart={()=>{engine.current.scrubbing=true}} onScrubEnd={()=>{engine.current.scrubbing=false}} label={selectionLabel||'Aircraft over Europe'} ariaLabel="Time of day UTC" variant={chartStyle} formatValue={formatActivity} description="5-minute snapshots · UTC"/>
