...
function Study({release}:{release:Release}) {
 ...
 useEffect(()=>{
  map.current?.setFlightRoute();setRouteState('loading')
  if(!flight)return
  const controller=new AbortController()
  void loadFlightRoute(flight,controller.signal).then(route=>{if(!controller.signal.aborted){map.current?.setFlightRoute(route);setRouteState('ready')}}).catch(()=>{if(!controller.signal.aborted)setRouteState('error')})
