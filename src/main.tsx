...
function Study({release}:{release:Release}) {
 const {manifest,index,land,snapshots,countries}=release,canvas=useRef<HTMLCanvasElement>(null),accentCanvas=useRef<HTMLCanvasElement>(null),map=useRef<AirMap|null>(null)
 ...
 const selectedAircraft=useMemo(()=>index.aircraft.filter(track=>matchesSelection(track,selection,countryIds)),[index,selection,countryIds])
 ...
 const selectionLabel=[selectedCountries.map(c=>c.name).join(' + '),airlineLabel(selection.airlines),selectedAirports.map(a=>a.iata||a.icao).join(' + '),selection.routes.map(id=>routes.find(r=>r.id===id)!.label).join(' + ')].filter(Boolean).join(' · ')
 ...
 return <main className="study">
  ...
  <div className="workspace">
   ...
   <aside aria-label="Flight selection">
    <FlightSearch countries={countries} airports={index.airports} routes={routes} selection={selection} onChange={changeSelection} onAirportBoard={choose} onCountryFocus={code=>map.current?.frameCountries([code])}/>
