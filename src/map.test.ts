...
it('feeds retained GPU state the shared positions, gap validity, selection and projection',()=>{
 ...
 expect(painter.prepare).toHaveBeenCalledWith(gapped,[],undefined,undefined,undefined)
 ...
 expect(painter.prepare).toHaveBeenLastCalledWith(gapped,[],'gapped',undefined,undefined)
...
it('dims context behind matches while keeping counts and picking scoped to the filter, including paused changes',()=>{
 ...
 expect(painter.prepare).toHaveBeenCalledWith(all,[],undefined,matching,undefined)
