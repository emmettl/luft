...
test('label boxes avoid each other and fixed UI; zoom reveals more airports without per-clock relayout',async({page})=>{
 ...
 const overlap=await page.evaluate(()=>{
  ...
  const controls=[...document.querySelectorAll('.study>header .identity,.study>header .date,.workspace aside,.map-caption,.view-controls,.study>footer')].map(e=>e.getBoundingClientRect())
