...
test('follow starts playback with an altitude profile and releases on manual map navigation',async({page})=>{
 await page.goto('./');await openBoard(page)
 await page.locator('.ms-split-flap-board__select').first().click()
 await expect(page.locator('.flight-caption')).toHaveAttribute('data-route-state','ready')
 await page.getByRole('button',{name:'Close airport board'}).click()
 await expect(page.getByRole('img',{name:/Observed altitude profile/})).toBeVisible()
 await page.getByRole('button',{name:'Follow aircraft',exact:true}).click()
 const following=page.getByRole('button',{name:'Following · Stop',exact:true})
 await expect(following).toHaveAttribute('aria-pressed','true')
 await expect(page.getByRole('button',{name:'Pause',exact:true})).toBeEnabled()
 await page.getByRole('button',{name:'Pause',exact:true}).click()
 await expect(following).toHaveAttribute('aria-pressed','true')
 await page.getByRole('button',{name:'Zoom in',exact:true}).click()
 await expect(page.getByRole('button',{name:'Follow aircraft',exact:true})).toHaveAttribute('aria-pressed','false')
 await expect(page.locator('.flight-caption')).toBeInViewport()
 await page.getByRole('button',{name:'Follow aircraft',exact:true}).click()
 await page.getByRole('button',{name:'Fit route',exact:true}).click()
 await expect(page.getByRole('button',{name:'Follow aircraft',exact:true})).toHaveAttribute('aria-pressed','false')
