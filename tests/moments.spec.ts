...
test('moments use recorded counts and jump from density to paused motion without losing filters',async({page})=>{
 ...
 await expect(panel.locator('.moment:not([data-moment$="wave"])')).toHaveCount(4)
...
test('airport scenes play to a bounded ending and release when the timeline is scrubbed',async({page})=>{
 await page.goto('./');await pauseAtStart(page)
 await page.getByRole('button',{name:/Moments.*to explore/}).click()
 await page.getByRole('button',{name:/^Watch arrivals into/}).click()
 const scene=page.getByRole('region',{name:'Guided moment'})
 await expect(scene).toBeVisible();await expect(scene).toContainText('Watching')
 await expect(page.getByRole('button',{name:'Pause',exact:true})).toBeEnabled()
 await expect(page.getByRole('combobox',{name:'Playback speed'})).toHaveValue('60')
 await page.getByRole('combobox',{name:'Playback speed'}).selectOption('900')
 await expect(scene).toContainText('Scene complete',{timeout:45000})
 await expect(page.getByRole('button',{name:'Play',exact:true})).toBeEnabled()
 await page.getByRole('button',{name:'Play',exact:true}).click()
 await expect(scene).toContainText('Watching')
