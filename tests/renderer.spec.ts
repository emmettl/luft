...
test('renderer switches preserve clock, carrier and airport; GPU draws and falls back after context loss',async({page})=>{
 ...
 await expect(page.locator('.diagnostics')).toContainText(/[1-6] GPU draw calls/)
 ...
 const copied=await page.evaluate(()=>JSON.parse((window as any).__copiedResults));expect(copied.renderer).toBe('three');expect(copied.release).toBe('0.2.0');expect(copied.gpu.calls).toBeGreaterThan(0)
